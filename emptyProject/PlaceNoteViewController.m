...
- (void) changeZoomWith:(int)zoomLevel {
    ...
    if (circleView) {
      ...
      int resultMOrKm = result>=1000?lroundf((float)result/1000):result;
        NSMutableString *strFormat = [NSMutableString stringWithFormat:@"In radius: %d ",resultMOrKm];
        if (result>=1000) {
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
    ...
    if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation)) {
        [circleRadiusView setFont:[UIFont fontWithName:@"HelveticaNeue-Light" size:12]];
    }
...
- (void) addCircleView {
    ...
    circleView.backgroundColor = [UIColor colorWithRed:0 green:0.5 blue:1 alpha:0.2];
    ...
    [circleRadiusView setFont:[UIFont fontWithName:@"HelveticaNeue-Light" size:17]];
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)aSearchBar {
    ...
    //[aSearchBar setShowsSearchResultsButton:YES];
}
